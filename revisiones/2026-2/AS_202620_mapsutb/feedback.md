...
La etiqueta `corte-1` sigue apuntando al commit inicial de la semana 1, por lo que deja fuera todo el avance actual y no constituye la entrega del reto. En ese estado no hay ADR, código, pruebas, medición ni arranque reproducible. Antes del cierre, ubiquen la etiqueta en el commit real del corte y completen el diagnóstico con línea base medible, ADR del reto, cambio aplicado, prueba específica, resultado contra el umbral y trazabilidad navegable. En el estado actual también deben alinear `docs/aspectos.md` con el alcance sin realidad aumentada, corregir la estructura `docs/arc42/` y `docs/c4/`, renombrar el ADR 0002 con la convención, dejar de reescribir ADR aceptados, registrar el uso de IA de la semana y excluir `.dart_tool/`. La restricción asignada, el PDF y la sustentación quedan pendientes de verificación docente.

## Semana 5 · Primer corte (revisión definitiva, 2026-09-07)

La etiqueta `corte-1` sigue apuntando al mismo commit de la semana 1: en la semana que tuvieron entre la revisión preliminar y el cierre no se movió. Eso significa que, formalmente, lo que se calificó es el estado de la semana 1 y no hay diagnóstico, ADR del reto, cambio, prueba ni medición que evaluar ahí. En el estado actual del repositorio sí hay avance real (un ADR de patrones de diseño y un primer flujo de ubicación en tiempo real), pero la tabla de trazabilidad de aspectos sigue sin actualizarse para reflejarlo, y el ADR anterior fue reescrito varias veces en lugar de reemplazarse con uno nuevo. Sigue sin existir un pipeline de integración continua. No se encontró ningún documento de correcciones del equipo a la revisión preliminar. Antes de la sustentación: muevan la etiqueta, escriban el diagnóstico y la medición del reto, y actualicen la cadena de trazabilidad para que cada fila lleve a algo real.
