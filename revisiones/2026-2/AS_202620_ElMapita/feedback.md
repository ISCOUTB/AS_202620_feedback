...
El contrato OpenAPI 3.1 versionado y el ADR de integración son un buen avance: la decisión síncrona frente a AsyncAPI está bien argumentada. Para cerrar el ciclo: 1) unifiquen el prefijo de rutas, hoy el backend expone /api/api/v1/... y el contrato y el cliente usan /api/v1/..., lo que deja la correspondencia contrato-código en no conformidad; 2) aporten el run de CI que ejecuta la prueba de contrato, con la línea del workflow, y una ejecución en rojo provocada por un cambio incompatible: sin ella no se demuestra que la prueba sirva; 3) sumen la evidencia de SonarCloud (configuración, run y URL pública con Quality Gate); 4) incluyan en la entrega el fragmento del contrato con rutas y esquemas, y las secciones arc42 §6 y C4 nivel 2 para revisar el etiquetado de protocolo y formato. Con esos cuatro puntos la entrega queda defendible de punta a punta.
## Semana 6 · S6
