...
El repositorio está ordenado: ADRs, arc42 con vista de ejecución, aspectos y registro de uso de IA se sostienen bien. Para cerrar la semana 7 falta lo central: un contrato de API en OpenAPI o AsyncAPI versionado en el repositorio, no solo la documentación que FastAPI genera en /docs, e incluyendo rutas y esquemas de datos. Añadan una prueba de contrato y hagan que el workflow la ejecute, y demuestren con un run en rojo que la prueba detecta un cambio incompatible. Registren la versión de la API y su historial en git. El ADR de integración debe decidir síncrono o asíncrono frente al escenario de calidad, con la alternativa descartada y su consecuencia de acoplamiento. En el C4 nivel 2 etiqueten cada flecha con protocolo y formato, y completen la tabla de aspectos con las columnas ID y C4. Por último, el análisis estático no es auditable hoy: agreguen el paso del scanner y la URL pública del Quality Gate.
