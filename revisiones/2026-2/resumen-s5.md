# Resumen de revision · Semana 5 · corte1 (definitive)

Generado por el pipeline automatico (GitHub Actions).

Nota sugerida = 1 + 4 × (n/m) sobre la matriz de la ficha, **propuesta al docente**.

| Equipo | Repo | Hash | n/m | Nota sugerida |
|---|---|---|---|---|
| AudioShare | `AS_202620_AudioShare` | `cb65d13` | 8/12 | 3.7 |
| Clubs UTB | `AS_202620_Clubs_UTB` | `4ede977` | 8/12 | 3.7 |
| DinamikUTB | `AS_202620_DinamikUTB` | `72bfc7e` | 8/12 | 3.7 |
| Drift | `AS_202620_Drift` | `74337a3` | 7/12 | 3.3 |
| ElMapita | `AS_202620_ElMapita` | `b28e068` | 7/12 | 3.3 |
| EnAgenda | `AS_202620_EnAgenda` | `696882e` | 10/12 | 4.3 |
| GimnasioUTB | `AS_202620_GimnasioUTB` | `-` | - | - |
| InvenTrack | `AS_202620_InvenTrack` | `ac951e3` | 9/12 | 4.0 |
| LaPlacita | `AS_202620_LaPlacita` | `50b92f8` | 8/12 | 3.7 |
| LostVault | `AS_202620_LostVault` | `c0c17c1` | 7/12 | 3.3 |
| CampusMarket | `AS_202620_PROYECTO_CAMPUSMARKET` | `8044215` | 10/12 | 4.3 |
| PideUtb | `AS_202620_PideUtb` | `bbefae8` | 9/12 | 4.0 |
| ROUTB | `AS_202620_ROUTB` | `343bb9d` | 8/12 | 3.7 |
| Recobra | `AS_202620_Recobra` | `f7c1a6c` | 8/12 | 3.7 |
| ShareU | `AS_202620_ShareU` | `19ce719` | 5/12 | 2.7 |
| Calificación automática | `AS_202620_Sistema-de-calificacion-automatica` | `8b0d00b` | 8/12 | 3.7 |
| TAIA | `AS_202620_TAIA_-Task-Artificial-Intelligence-Assistant` | `-` | - | - |
| Tienda virtual UTB | `AS_202620_TIENDA-VIRTUAL-UTB` | `3d732d7` | 9/12 | 4.0 |
| TRACTAR | `AS_202620_TRACTAR` | `7cfb872` | 8/12 | 3.7 |
| Verifacts | `AS_202620_Verifacts` | `67f8cea` | 8/12 | 3.7 |
| XALD | `AS_202620_XALD` | `9bf16cf` | 8/12 | 3.7 |
| mapsutb | `AS_202620_mapsutb` | `8aee879`* | 8/12 | 3.7 |
| uniTeam | `AS_202620_uniTeam` | `-` | - | - |

\* Revisión manual excepcional: se aceptaron las correcciones aplicadas durante la semana, posteriores al cierre original.
