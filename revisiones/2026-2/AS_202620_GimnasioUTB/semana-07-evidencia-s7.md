...
| Correspondencia entre el contrato y la API implementada | Solo el README cita `POST /api/v1/aforo/acceso` y `GET /api/v1/aforo`; no se aportó el contenido de src/modules/aforo/infrastructure/http/aforo.router.js. | No verificado | Haría falta citar dos rutas del contrato en el router y una ruta del código presente en el contrato. |
| Versión de la API declarada y con historial | docs/openapi.yaml declara `info.version: 1.0.0`, pero no se aporta `git log` del archivo. | No verificado | La versión está declarada; falta el historial de cambios del contrato. |
...
| Evidencia de que la prueba falla ante un cambio incompatible | Cuatro runs CI con conclusion failure el 2026-09-21 entre 01:23Z y 01:27Z, p. ej. https://github.com/ISCOUTB/AS_202620_GimnasioUTB/actions/runs/35550973353, seguidos de runs en success. | Cumple | Existe run en rojo, pero no se aporta el log del paso fallido que confirme que el fallo vino del cambio incompatible. |
...
| arc42 sección 6 con los flujos de interacción | El extracto de docs/arc42/arc42_gimnasio_utb.md se corta en la sección 3.2; solo hay enlaces a `#6-runtime-view-vista-de-ejecución` (ADR-0003) y la mención en correcciones.md. | No verificado | Falta el contenido de la sección 6 y sus diagramas de secuencia. |
| C4 nivel 2 con protocolo y formato en cada flecha | docs/c4/c4_level2.md etiqueta 4 de 6 flechas con protocolo/formato (HTTPS REST / JSON, TCP 5432 / SQL, FCM Push / JSON); las flechas estudiante→app y encargado→app solo dicen «Interacción UI / …». | No cumple | Falta protocolo y formato en las dos flechas de actor a contenedor. |
...
- Resumen: En HEAD 0e3aeb5 (origin/main, 2026-09-21T04:29Z, dentro del cierre) el equipo entrega contrato OpenAPI 3.1.0 con esquemas, prueba de contrato invocada por ci.yml y runs en verde; siguen pendientes la evidencia de SonarCloud exigida desde S6, el ADR de integración sin alternativa descartada, el ADR 0001 duplicado y la tabla de aspectos sin las ocho columnas.
...
- C4 nivel 2 con dos flechas de actor a contenedor sin protocolo ni formato.
- Verificar correspondencia contrato↔código y el historial de versiones del contrato.
- Verificar el contenido de la sección 6 del arc42.
...
- Correspondencia contrato↔API: no se aportó el contenido de src/modules/aforo/infrastructure/http/aforo.router.js; haría falta citar sus rutas (POST /api/v1/aforo/acceso, GET /api/v1/aforo) y cruzar una ruta del código con el contrato.
- Historial del contrato: falta `git log --format='%h %cI %s' -- docs/openapi.yaml`.
- arc42 sección 6: el extracto se corta en la sección 3.2; haría falta el contenido de la vista de ejecución con los diagramas de secuencia.
...
- docs/openapi.yaml declara version 1.0.0 y tres rutas con esquemas, pero no se aporta su git log.
- docs/aspectos.md usa columnas de escenarios de calidad en vez de las ocho columnas del curso.
- c4_level2.md etiqueta con protocolo y formato 4 de 6 flechas; las de actor a app no los declaran.
