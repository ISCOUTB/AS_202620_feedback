...
| Versión de la API declarada y con historial | `info.version: 1.0.0` en docs/contracts/lostvault-api.yaml; el archivo aparece en el árbol de HEAD. | No verificado | La evidencia no incluye el historial del archivo; haría falta `git log --format='%h %cI %s' -- docs/contracts/lostvault-api.yaml`. |
...
| Pipeline y análisis estático | .github/workflows/build.yml invoca SonarSource/sonarqube-scan-action con SONAR_TOKEN y existe sonar-project.properties; no se aporta URL pública del análisis en SonarCloud ni estado del Quality Gate, y el run «Build» más reciente falló (35562739717, 2026-09-21T04:57:04Z). | No cumple | Faltan la segunda y tercera evidencias exigidas (run del scanner para el hash y análisis público con Quality Gate); se esperaba además que el pipeline bloqueara ante el fallo. |
...
- URL pública del análisis en SonarCloud con Quality Gate para el hash revisado y run «Build» en verde.
...
- Historial de git del contrato: falta `git log --format='%h %cI %s' -- docs/contracts/lostvault-api.yaml`.
...
- El run «Build» (SonarQube) más reciente de la rama terminó en failure.
