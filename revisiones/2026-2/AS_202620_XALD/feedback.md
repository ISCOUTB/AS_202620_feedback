...
El contrato está bien redactado: versión declarada, rutas y esquemas completos, y las decisiones se sostienen con ADR y arc42. Lo que falta es justamente lo que da valor a la semana: una prueba de contrato que el pipeline ejecute y la evidencia de que se pone en rojo cuando el proveedor rompe el contrato. Sugerencias concretas: (1) añadir la prueba que valide el contrato contra el backend y provocar a propósito un cambio incompatible, guardando el run fallido; (2) alinear la ruta del contrato con la que realmente usa la sincronización y documentar la correspondencia en ambos sentidos; (3) dejar en el workflow la línea que invoca el scanner y publicar la URL del análisis con su Quality Gate; (4) completar el C4 nivel 2 etiquetando cada flecha con protocolo y formato. Con esos cuatro ajustes el contrato deja de ser documentación y pasa a ser una garantía verificable.
## Semana 6 · S6
