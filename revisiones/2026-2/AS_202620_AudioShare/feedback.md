...
La entrega llega sin el contrato de la API y eso arrastra casi toda la matriz de la semana. Un JSON/YAML OpenAPI versionado, con rutas y esquemas de respuesta, es el punto de partida. Sobre él, agreguen una prueba de contrato que corra en el pipeline y demuestren que falla al introducir un cambio incompatible: guarden el run en rojo o el caso que la rompe, porque una prueba que nunca falla no prueba nada. Falta también el ADR que justifique la estrategia de integración (síncrona o asíncrona) contra un escenario de calidad, con la alternativa descartada. Lo que sí está bien: la sección 6 de arc42 y el C4 nivel 2 con protocolo y formato en cada flecha. Completen además la evidencia de análisis estático con configuración, run y URL pública con Quality Gate. Con esos faltantes resueltos, las entregas anteriores quedan bien soportadas.
