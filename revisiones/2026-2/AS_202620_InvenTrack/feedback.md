...
El repositorio muestra avance real: el contrato está versionado en el repositorio, existe una prueba que lo compara con la API y un ADR que justifica la integración síncrona frente a la alternativa por eventos. Para que esta entrega sea verificable: 1) incluyan la línea del workflow que ejecuta la prueba de contrato y la URL del run del hash entregado; 2) aporten la evidencia de que la prueba falla al introducir un cambio incompatible (run en rojo o el cambio que la hizo fallar); 3) confirmen que el contrato declara versión y esquemas de datos y que sus rutas corresponden a las implementadas; 4) etiqueten cada flecha del C4 nivel 2 con protocolo y formato (por ejemplo, HTTPS/JSON); 5) reactiven el análisis estático en CI y compartan la URL pública con el estado del Quality Gate, porque sin ejecución no cuenta como evidencia. Con esos cinco puntos la entrega pasa de 'existe' a 'está demostrado'.
