...
El contrato OpenAPI 3.1 está versionado y se corresponde con la superficie implementada, y el ADR-0002 y la sección 6 de arc42 sostienen bien la estrategia síncrona. Para cerrar la semana falta hacer auditable lo que ya existe: subir el contenido de la evidencia del cambio incompatible (o enlazar el run en rojo que provoca), confirmar en el workflow la línea que ejecuta la prueba de contrato y aportar la URL pública del análisis con su Quality Gate. Conviene también completar el C4 nivel 2 con protocolo y formato en cada flecha y dejar el historial del contrato en el informe. Con esos enlaces, el trabajo pasa de 'documentado' a 'verificable'.
